...
-(void) appendElement:(id<IVTRichElement>) element text:(NSString *) text attributes:(NSDictionary *) attributes{
    ...
    if([attr valueForKey:(id)kCTParagraphStyleAttributeName] == nil){
        ...
        [attr setValue:(__bridge id)style forKey:(__bridge id)kCTParagraphStyleAttributeName];
        
        CFRelease(style);
        
    }
    
    CTFontRef font = (__bridge CTFontRef)[attr valueForKey:(id)kCTFontAttributeName];
    ...
    CGColorRef textColor = (__bridge CGColorRef)[attr valueForKey:(id)kCTForegroundColorAttributeName];
...
-(void) drawContext:(CGContextRef) context withSize:(CGSize) size{
    ...
    for(lineIndex = 0;lineIndex < count; lineIndex ++){
        ...
        for(int i=0;i<CFArrayGetCount(runs);i++){
            ...
            CGColorRef bgColor = (CGColorRef) CFDictionaryGetValue(CTRunGetAttributes(run), (__bridge const void *)(VTBackgroundColorAttributeName));
